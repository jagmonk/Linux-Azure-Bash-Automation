# Azure Blob Backup Sync

## Purpose

Sync a local Linux backup folder to Azure Blob Storage with dry-run and explicit apply mode.
...
```bash
chmod +x scripts/blob-backup-sync.sh
./scripts/blob-backup-sync.sh
```
...
Built a Bash-based azure blob backup sync automation project using Azure CLI, jq, structured reporting, polished headers, and operational documentation for hybrid cloud Systems Administration.
